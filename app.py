...
<head>
    ...
    <style>
        * { -webkit-tap-highlight-color: transparent; }
        ...
        /* Dark overlay on top of the image so text is readable */
        #unlock-overlay-bg { position: absolute; inset: 0; background: rgba(0,0,0,0.7); backdrop-filter: blur(3px); z-index: -1; }
        
        .unlock-btn { background: linear-gradient(45deg, #db2777, #ec4899); color: white; padding: 12px 30px; border-radius: 30px; font-weight: 900; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; box-shadow: 0 4px 15px rgba(219, 39, 119, 0.5); animation: pulse 2s infinite; z-index: 10; }
        ...
        <div class="relative w-full aspect-video bg-black sticky top-0 z-50">
            ...
            <div id="video-container" class="w-full h-full relative bg-black">
                ...
                <div id="unlock-screen">
                    ...
                    <div class="text-pink-500 text-4xl mb-2 z-10"><i class="fa fa-lock"></i></div>
                    <h2 id="lock-msg" class="text-white font-bold text-sm mb-6 tracking-widest z-10 text-shadow">EPISODE LOCKED</h2>
                    <button onclick="unlockEpisode()" class="unlock-btn">
                        <i class="fa fa-play mr-2"></i> TAP TO UNLOCK
                    </button>
                    <p class="text-[10px] text-zinc-300 mt-4 z-10 font-bold">Free Stream • Ad Supported</p>
                </div>
    ...
    <script>
        ...
        function switchEpisode(s, e, btn) {
            ...
            if(watchedAds.has(currentTvId)) {
                ...
                showLockScreen(e); // Update text to "UNLOCK EPISODE X"
            }
        }
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000, threaded=True)            tmp = CACHE_FILE + ".tmp"
            with open(tmp, 'w') as f: json.dump(data, f)
            os.replace(tmp, CACHE_FILE)
