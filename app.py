...
<head>
    ...
    <style>
        * { -webkit-tap-highlight-color: transparent; }
        ...
        #unlock-overlay-bg { position: absolute; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(12px); z-index: -1; }
        
        .unlock-btn { background: linear-gradient(45deg, #db2777, #ec4899); color: white; padding: 15px 35px; border-radius: 40px; font-weight: 900; font-size: 16px; text-transform: uppercase; letter-spacing: 1px; box-shadow: 0 4px 15px rgba(219, 39, 119, 0.5); animation: pulse 2s infinite; z-index: 10; border: 2px solid rgba(255,255,255,0.2); transition: all 0.3s ease; }
...
<body class="pb-32">
        ...
        <div class="relative w-full aspect-video bg-black sticky top-0 z-50">
            ...
            <div id="video-container" class="w-full h-full relative bg-black">
                ...
                <div id="unlock-screen">
                    ...
                    <button id="btn-unlock-action" onclick="unlockEpisode()" class="unlock-btn hover:scale-105 active:scale-95">
    ...
    <script>
        ...
        function preparePlayer(id) { 
            // FIXED: Force loose comparison or string conversion for safety
        ...
        async function openPlayer(item) {
            ...
            currentTvId = String(item.id); // FIXED: Store as String
        ...
        function showLockScreen(epNum) {
            ...
            // RESET BUTTON STATE (Important Fix)
            btn.innerHTML = '<i class="fa fa-play mr-2"></i> TAP TO UNLOCK';
            btn.style.background = ''; // Reset gradient
            btn.dataset.loading = "false";
            btn.style.opacity = "1";

            // SHOW SCREEN / HIDE VIDEO
            vFrame.classList.add('hidden');
            vFrame.src = ""; 
            
            screen.classList.remove('hidden');
            screen.style.display = 'flex'; // Enforce Flex
        ...
        function showVideoDirectly() {
            ...
            screen.style.display = 'none'; // Explicitly remove from flow
        ...
        function unlockEpisode() {
            ...
            // 3. Simulate Ad Wait
            setTimeout(() => {
                watchedAds.add(String(currentTvId)); // FIXED: Add as String
        ...
        function switchEpisode(s, e, btn) {
            ...
            if(watchedAds.has(String(currentTvId))) {
                showVideoDirectly(); // Ensure lock screen is hidden
                refreshVideoSource();
            } else {
                showLockScreen(e);
            }
        ...
        function saveToLib() { let s=JSON.parse(localStorage.getItem('kcity_saved')||'[]'); if(!s.find(i=>String(i.id)===String(currentItem.id))) { s.unshift(currentItem); localStorage.setItem('kcity_saved', JSON.stringify(s)); alert("Saved to Profile!"); } }
